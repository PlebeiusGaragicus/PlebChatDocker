from langserver.src.BaseBot import BaseBot

class PlebChat(BaseBot):
    ...
    def whoami(self, request, *args):
        """Get your username."""
        return "Your username is user123"

    def debug(self, request, *args):
        """Get debug information."""
        debug_info = "Debug info here"
        if args:
            debug_info += f" with parameters: {' '.join(args)}"
        return debug_info

    def long_message(self, request, *args):
        """Get a long message."""
...
# def draw(_):
#     from src.test_graph.graph import GRAPH_ASCII
#     return f"```\n{GRAPH_ASCII}\n```"
...
#     # COMMANDS CAUGHT BY THE LANGSERVER
